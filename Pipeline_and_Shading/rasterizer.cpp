...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    int xmin = std::floor(std::min({v[0].x(), v[1].x(), v[2].x()}));
    int xmax = std::ceil(std::max({v[0].x(), v[1].x(), v[2].x()}));
    int ymin = std::floor(std::min({v[0].y(), v[1].y(), v[2].y()}));
    int ymax = std::ceil(std::max({v[0].y(), v[1].y(), v[2].y()}));
        ...
        for(int y = ymin; y < ymax; ++y){
            if(insideTriangle(x+0.5, y+0.5, t.v)){
                ...
                if(depth_buf[ind] > zp){
                    ...
                    depth_buf[ind] = zp;
                    Eigen::Vector2i Point = Vector2i(x, y);
